(function () {
  ...
  const setAlert = (message, tone = "error") => {
    ...
    alertEl.classList.toggle("card-overlay__alert--success", tone === "success");
    alertEl.classList.toggle("card-overlay__alert--error", tone !== "success");
  ...
  const renderCard = ({ card, activities }) => {
    ...
    createdEl.textContent = card.createdAt
      ...
      : "—";
    balanceEl.textContent = formatter.format((card.balance.amount || 0) / 100);
    blockBtn.hidden = card.state === "BLOCKED" || card.state === "DEACTIVATED";
